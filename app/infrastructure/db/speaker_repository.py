...
from app.infrastructure.db.models import SpeakerORM
...
class PostgresSpeakerRepository:
    ...
    def get_by_meeting_and_label(
        self, meeting_id: UUID, diarization_label: str
    ) -> Optional[Speaker]:
        row = self._session.scalar(
            select(SpeakerORM).where(
                SpeakerORM.meeting_id == meeting_id,
                SpeakerORM.diarization_label == diarization_label,
            )
        )
        return _speaker_to_domain(row) if row else None

...
class Formatter:
    def merge_segments(self, segments: list[SpeechSegment], max_gap_ms: int = 3000) -> list[SpeechSegment]:
        if not segments:
            return []
    ...
    def to_txt(self, segments: list[SpeechSegment]) -> str:
        ...
        for segment in segments:
            speaker = segment.person_name if segment.person_name != "Unknown" else segment.speaker_id
    ...
    def to_html_preview(
        ...
        if len(segments) > max_segments:
            more = (
                ...
                "Полная стенограмма находится во вложении.</i></p>"
            )

        return f"""
        <html>
        <body>
            <h2>Стенограмма встречи готова</h2>
